...
def fetch_and_save_manga_from_mangadex():
    ...
    try:
        # Lấy danh sách manga
        response = requests.get(f"{base_url}/manga", params={"limit": 50})
...
def search_by_emotion():
    ...
    # Filter manga based on keywords in the content
    result = [
        {
            "title": manga[0],  # Assuming title is in the first position
            "content": manga[1],  # Assuming content is in the second position
            "cover_image": manga[2],  # Assuming cover_image is in the third position
        }
